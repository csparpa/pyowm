...
class LRUCache(OWMCache):   
    ...
    def get(self, request_url):
        ...
        try:
            ...
            if current_time - cached_item['insertion_time'] > self.__item_lifetime:
                # Cache item has expired
                self.__clean_item(request_url)
                return None
            cached_item['insertion_time'] = current_time  #Update insertion time
    ...
    def set(self, request_url, response_json):
        ...
        :param request_url: the request URL that uniquely identifies the request
            whose response is to be cached
        :type request_url: str
