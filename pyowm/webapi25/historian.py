...
class Historian(object):
    ...
    history data. The class encapsulates a *StationHistory* instance and provides
    abstractions on the top of it in order to let programmers exploit meteostation
    weather history data in a human-friendly fashion
    ...
    def temperature_series(self, unit='kelvin'):
        ...
        for timestamp in self.__station_history.get_measurements():
            t = self.__station_history.get_measurements()[timestamp]['temperature']
            if unit == 'kelvin':
                temp = t
            if unit == 'celsius':
    ...
    def humidity_series(self):
        ...
        """
        return [(timestamp, \
                self.__station_history.get_measurements()[timestamp]['humidity']) \
                for timestamp in self.__station_history.get_measurements()]
    ...
    def pressure_series(self):
        ...
        """
        return [(timestamp, \
                self.__station_history.get_measurements()[timestamp]['pressure']) \
                for timestamp in self.__station_history.get_measurements()]
    ...
    def rain_series(self):
        ...
        """
        return [(timestamp, \
                self.__station_history.get_measurements()[timestamp]['rain']) \
                for timestamp in self.__station_history.get_measurements()]
